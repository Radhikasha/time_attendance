...
exports.checkIn = async (req, res) => {
    ...
    try {
        ...
        if (existingAttendance) {
            ...
            // NEW: Also prevent check-in if user has already completed attendance
            if (existingAttendance.checkIn && existingAttendance.checkOut) {
                return res.status(400).json({ 
                    success: false,
                    message: 'You have already completed attendance for today. Next check-in is available tomorrow.',
                    attendance: existingAttendance
                });
            }
        }
