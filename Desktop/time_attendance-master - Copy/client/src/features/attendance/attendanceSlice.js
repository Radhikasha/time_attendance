...
export const getTodaysAttendance = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await attendanceAPI.getTodaysAttendance();
      return response.data.data; // Based on server response structure
    } catch (error) {
...
export const checkIn = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      // Return the attendance object, not the wrapper
      return response.data.attendance || response.data;
    } catch (error) {
      console.error('❌ Check-in API error:', error.response?.data);
